<template>
  <div style="background:#f2f2f2;">
    ...
    <card style="margin: 20px">
      ...
      <div style="font-size: 28px">二、报表区域：（如全省）</div>
      <div style="font-size: 28px">三、报表数据</div>
      <div style="font-size: 28px">3.1 旅游热度概况</div>

      <div v-if="c.indexOf('1')>-1">(1) 游客人数
        <div style="color: red">（文字表述，无图表）</div>
        <div>{{FlowCity}}总接待游客量为{{tourPeople.total}}人次，（如选择日，则有以下内容：与昨</div>
        <div>日环比{{tourPeople.ratio}}），与上月同比{{tourPeople.link}}。</div>
      </div>

      <div v-if="c.indexOf('2')>-1">(2) 区域游客占比
        <!--<div style="color: red">（文字描述区域占比TOP5，图表展示所有区域占比状况）</div>-->
        <div> （区域）接待游客占比量前5分别为： （区域/景区名称） ， </div>
        <div>（排名第一{{areaPeople[0].name}}）接待人数约{{areaPeople[0].value}}万人，占比总量{{areaPeople[0].proportion}}%;</div>
        <div>（排名第二{{areaPeople[1].name}}）接待人数约{{areaPeople[1].value}}万人，占比总量{{areaPeople[1].proportion}}%;</div>
        <div>（排名第三{{areaPeople[2].name}}）接待人数约{{areaPeople[2].value}}万人，占比总量{{areaPeople[2].proportion}}%;</div>
        <div>（排名第四{{areaPeople[3].name}}）接待人数约{{areaPeople[3].value}}万人，占比总量{{areaPeople[3].proportion}}%;</div>
        <div>（排名第五{{areaPeople[4].name}}）接待人数约{{areaPeople[4].value}}万人，占比总量{{areaPeople[4].proportion}}%;</div>

      </div>
     <!-- <div style="width: 400px;height: 500px;">
        <indexMap :mapdata="this.areaPeople1" style="width: 100%;height: 500px;"></indexMap>
      </div>-->
      <div v-if="c.indexOf('3')>-1">(3) 游客趋势（若日期选择今日、昨日，则无此模块）
        <div style="color: red">文字描述客流最大量、最小量，图表展示趋势图）</div>
        <div>{{trendPeople[0].date}}客流量最大，为{{trendPeople[0].value}}万人；{{trendPeople[trendPeople.length-1].date}}客流量最小，为{{trendPeople[trendPeople.length-1].value}}万人。</div>
      </div>
      <div>11</div>

      <div style="font-size: 28px">3.2 核心景区排行（省级/州市支持导出对应区域排行数据）</div>
      <div v-if="c1.length>0">
        <div style="color: red">（文字描述景区客流TOP5，图表展示所有5A/4A级景区排行详情，若未选择5A/4A，则默认输出景区排行TOP10）</div>
        <div>景区累计游客人数排行前五分别为：</div>
        <div> {{coreSenic[0].name}}，接待游客人{{coreSenic[0].cur}}万人次；</div>
        <div> {{coreSenic[1].name}}，接待游客人{{coreSenic[1].cur}}万人次；</div>
        <div> {{coreSenic[2].name}}，接待游客人{{coreSenic[2].cur}}万人次；</div>
        <div> {{coreSenic[3].name}}，接待游客人{{coreSenic[3].cur}}万人次；</div>
        <div> {{coreSenic[4].name}}，接待游客人{{coreSenic[4].cur}}万人次；</div>

     </div>
        <div style="font-size: 28px">3.3 景区指数排行</div>
         <div v-if="c2.indexOf('6')>-1">
           <div style="color: red">（文字描述各指数最大的景区及指数值，图表展示所有指数排行）</div>
     ...
     <div style="font-size: 28px">3.4 游客画像</div>
      ...
      <div v-if="c3.indexOf('7')>-1">(1) 基本画像
        <div style="color: red">（文字描述基本信息，多个字段时，描述最大占比，两个字段时，描述所有占比；图表展示所有画像）</div>
        <div>（区域/景区）来访游客中:</div>
        ...
        <div> {{parseInt(imgmobile[1].value*10000)/100}}%的游客手机终端类型为IOS，{{parseInt(imgmobile[0].value*10000)/100}}%的游客手机终端类型为Android；</div>
        <div> 市来源游客量最大，占比总游客的 %；</div>
        <div> 省来源游客量最大，占比总游客的 %。</div>
      </div>

      <!--  <div v-if="c3.indexOf('8')>-1">(2) 人口迁徙
       <div style="color: red">（文字描述迁入、迁出路线TOP1，图表展示迁徙路线TOP10）</div>
     ...
     <div v-if="c3.indexOf('9')>-1">(3) 一机游用户消费
       <div>一机游用户平均消费金额为{{avg.avg_amount}}元，用户在{{rank[0].name}}消费金额最高，为{{rank[0].avg}}元；用户消费中，景区门票消费占比{{cate[1].value}}%，酒店消费占比{{cate[2].value}}%，机票消费占比{{cate[0].value}}%。</div>
     </div>
      ...
      <!--  <div v-if="c3.indexOf('10')>-1">(3) 游客线下消费
        ...
        <div style="font-size: 28px">3.5 游客体验</div>
         ...
         <div v-if="c4.indexOf('12')>-1">(2) 投诉时长分析
           <div style="color: red">（文字表述，图表呈现）</div>
    ...
    methods: {
      init() {
        ...
        http.get('bi/get_tourism_dist_by_datespan', {
          ...
          this.areaPeople1=resp.data.hits.map(item=>{return{name:item.name,value:item.proportion}})
