...
<script>
  ...
  export default {
    ...
    methods: {
      //4.1.1游客人数
      get_tourism_qty_by_date(c,s,d){
        http.post('bi/get_tourism_qty_by_date',{city_id:c, scenic:s,date:http.gmt2str(d)}).then(resp => {
      ...
      //4.1.3客流统计
      get_tourism_trend_by_timespan(c,s,st,et){
        http.post("bi/get_tourism_trend_by_timespan", {city_id:c, scenic:s,startTime:http.gmt2str(st),endTime:http.gmt2str(et)}).then(resp => {
      ...
      getCity() {
        http.post("bi/get_all_city_prov", {}).then(resp => {
          ...
          http.post('bi/get_scenic_by_city', {city_id: this.city}).then(resp => {
            if (resp.data.errcode === 0) {
              ...
              this.get_tourism_trend_by_timespan(this.city1,this.citysenic2,this.date1[0],this.date1[1])
            }
          })
        });
        ...
        this.get_tourism_qty_by_date(this.city,null,this.datefff);
      },
      initBar() {
      ...
      form1change() {
        if (this.citysenic1=='') {
          ...
          this.get_tourism_qty_by_date(this.city,this.citysenic1,this.datefff);
        }else {
          this.isshowmap1=2
          this.get_tourism_qty_by_date(this.city,this.citysenic1,this.datefff);
      ...
      p2(val) {
        if (val == 1) {
          this.date1 = [http.if10(), http.if10()];
          this.get_tourism_trend_by_timespan(this.city1,this.citysenic2,this.date1[0],this.date1[1])
        }
        if (val == 2) {
